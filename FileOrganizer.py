...
def move_file(dest, entry, name):
    if exists(f"{dest}/{name}"):
        unique_name = unique_file(dest, name)
...
class MyFileMoverHandler(FileSystemEventHandler):
    ...
    def check_audio_files(self, entry, name):
        for audio_extension in audio_extension:
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO,
                        ...
                        datefmt='%Y-%m-%d %H:%M:%S')
    path = source_dir
    event_handler = MyFileMoverHandler()
    observer = Observer()
